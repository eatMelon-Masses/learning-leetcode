...
class Solution {
    ...
    final int[][] dp=new int[piles.length][piles.length];
        if(piles.length==0) return true;
    for(int i=0;i<=piles.length-1;i++){
        dp[i][i]=piles[i];
    }
    for(int j=1;j<=piles.length-1;j++){ 
        for(int k=0;k+j<=piles.length-1;k=k+j){
            dp[k][k+j]=Math.max(piles[k]-dp[k+1][k+j],piles[k+j]-dp[k][k+j-1]);
        }
    }
